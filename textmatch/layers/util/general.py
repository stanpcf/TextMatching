...
def exp_mask(val, mask, name=None):
    ...
    if name is None:
        name = "exp_mask"
    return K.tf.add(val, (1 - K.tf.cast(mask, 'float')) * VERY_NEGATIVE_NUMBER, name=name)


def flatten(tensor, keep):
    fixed_shape = tensor.get_shape().as_list()
    start = len(fixed_shape) - keep
    left = reduce(mul, [fixed_shape[i] or K.tf.shape(tensor)[i] for i in range(start)])
    out_shape = [left] + [fixed_shape[i] or K.tf.shape(tensor)[i] for i in range(start, len(fixed_shape))]
    flat = K.tf.reshape(tensor, out_shape)
    return flat
...
def reconstruct(tensor, ref, keep):
    ...
    pre_shape = [ref_shape[i] or K.tf.shape(ref)[i] for i in range(ref_stop)]
    keep_shape = [tensor_shape[i] or K.tf.shape(tensor)[i] for i in range(tensor_start, len(tensor_shape))]
    # pre_shape = [tf.shape(ref)[i] for i in range(len(ref.get_shape().as_list()[:-keep]))]
    # keep_shape = tensor.get_shape().as_list()[-keep:]
    target_shape = pre_shape + keep_shape
    out = K.tf.reshape(tensor, target_shape)
    # print("out shape")
    # print(out.get_shape())
    return out
